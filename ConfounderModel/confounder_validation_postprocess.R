...
## this file processes the confounder model validation run for MIMIC

setwd("C:/Git/PAHRC/imvThresholdsTargetTrial")

file <- paste0("C:/Users/chris/OneDrive/Documents/LargeDataFiles/predictions.rds")
  
vars <- data.frame(var = 1:17,
...
ggsave("plots/confounder_rmse.svg", plot_rmse, height = 6, width = 8)
ggsave("plots/confounder_coverage.svg", plot_cover, height = 6, width = 8)

contmean <- filter(bigdf, var <= 9) %>%
  group_by(timestep,sample,var) %>%
  summarise(mean_obs = mean(value),
...
ggsave("plots/confounder_cmean.svg", plot_cmean,
       height = 6, width = 8)

# Overall RMSEs 

mse_cov_tbl <- filter(bigdf, var <= 9) %>%
  mutate(err2 = (pred-value)^2) %>%
  group_by(timestep,sample,var) %>%
  summarise(mean_obs = mean(value),
            mean_pred = mean(pred),
...
contmean_tbl <- filter(bigdf, var <= 9) %>%
  group_by(timestep,sample,var) %>%
  summarise(mean_obs = mean(value),
            mean_pred_sample = mean(pred),
            count_sample_var = n()) %>%
...
ggsave("plots/confounder_binpred.svg", plot_binpred,
       width = 8, height = 6)
...
left_join(rocprc_tbl,bmean_tbl,by = "var") %>%
  ...
  write.csv(file = "ROCPRC_bmean_Tbl.csv")
...
left_join(mse_cov_tbl, contmean_tbl, by = "var") %>%
  ...
  write.csv(file = "RMSE_cmean_Tbl.csv")
...
plot_validation_trajectory <- function(plot_id){
  ...
  onept_df <- filter(bigdf, stay_id == plot_id) %>%
    group_by(time, var) %>%
...
plot_id <- sample(unique(bigdf$stay_id),1)
plot_validation_trajectory(plot_id)
ggsave(filename = paste0("plots/validation_trajectory", plot_id,".svg"),
       height = 6, width = 8)
