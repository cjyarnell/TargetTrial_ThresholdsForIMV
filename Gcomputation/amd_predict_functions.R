...
load_y_pred <- function(pt_admissionid){
  # load a trajectory
...
load_eta <- function(pt_admissionid){
  eta <- readRDS(paste0(dir, "/etasim",pt_admissionid, ".rds"))
  eta
}
...
rox_threshold <- function(traj, x){
  (rox(traj) < x) & (traj$niv == 1 |   traj$stdo2 == 1)
}
...
sf_threshold <- function(traj, x){
  (sf(traj) < x) & (traj$niv == 1 |   traj$stdo2 == 1)
}
...
delta_threshold <- function(traj, x){
    if(nrow(traj) == 1){
        ...
        return((sf(traj) + delta*x < 88) &
                (traj$niv == 1 |   traj$stdo2 == 1) &
                (lag(traj$niv) == 1 | lag(traj$stdo2 == 1)))
    }
}
...
hacor_threshold <- function(traj, x){
  (hacor(traj) > x) & (traj$niv == 1 |   traj$stdo2 == 1)
}
...
florali_threshold <- function(traj,x){
  florali(traj)  & (traj$niv == 1 |   traj$stdo2 == 1)
}
...
demontb <- function(traj){
  ...
  dmb_all <- dmb_sf & dmb_rr
  dmb_all
}

demontb_threshold <- function(traj,x){
  (demontb(traj) > 0)  & (traj$niv == 1 |   traj$stdo2 == 1)
}

# MOD

mod <- function(traj){
  mod_resp <- (sf(traj) < 120) 
...
mod_threshold <- function(traj,x){
  (mod(traj) > 0)  & (traj$niv == 1 |   traj$stdo2 == 1)
}
...
modAnd_threshold <- function(traj,x){
  (modAnd(traj) > 0)  & (traj$niv == 1 |   traj$stdo2 == 1)
}
