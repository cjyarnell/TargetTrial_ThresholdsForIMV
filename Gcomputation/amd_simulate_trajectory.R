...
files = c("confounderOutputs/amd_confounder_chain1.csv",
              "confounderOutputs/amd_confounder_chain2.csv",
              "confounderOutputs/amd_confounder_chain3.csv",
              "confounderOutputs/amd_confounder_chain4.csv")

fit <- read_cmdstan_csv(files = files,variables = c("rho","alpha","sigma","offset","L_C","beta_mu"),sampler_diagnostics = "", format = "draws_matrix")
saveRDS(fit, "data/amd_confounder_fit_GPparams.rds")
fit <- readRDS("data/amd_confounder_fit_GPparams.rds")

n_chains <- length(files) #fit$metadata$num_chains
...
for (pos in 1:use_iter){
  ...
  rho[[pos]]    <- as.numeric(fit$post_warmup_draws[pos+shift,grepl("rho",dimnames(fit$post_warmup_draws)$variable)])
  alpha[[pos]]  <- as.numeric(fit$post_warmup_draws[pos+shift,grepl("alpha*",dimnames(fit$post_warmup_draws)$variable)])
  sigma[[pos]]  <- as.numeric(fit$post_warmup_draws[pos+shift,grepl("sigma*",dimnames(fit$post_warmup_draws)$variable)])
  offset[[pos]] <- as.numeric(fit$post_warmup_draws[pos+shift,grepl("offset*",dimnames(fit$post_warmup_draws)$variable)])
  L_C[[pos]]    <- matrix(as.numeric(
    fit$post_warmup_draws[
      pos+shift,grepl("L_C*",dimnames(fit$post_warmup_draws)$variable)]), 
                          nrow = D, ncol = D)
  beta_mu[[pos]]<- matrix(as.numeric(
    fit$post_warmup_draws[
      pos+shift,grepl("beta_mu*",dimnames(fit$post_warmup_draws)$variable)]),
    ncol = D)
  pos <- pos+1
}

params = list(rho=rho, alpha=alpha, sigma=sigma, offset=offset, L_C=L_C, beta_mu=beta_mu)
...
simulate_dat <- readRDS("data/amd_simulate.rds")

# load more relevant stuff
mean_sd <- readRDS("data/amd_tt_mean_sd.rds") %>%
...
simulate_trajectory <- function(pt_admissionid){

# for building
# pt_admissionid <- 20
id <- filter(simulate_dat$patients, admissionid == pt_admissionid)$id    
...
B <- simulate_dat$B
dfb <- as.numeric(filter(simulate_dat$dfb, admissionid == pt_admissionid) %>% 
  select(-admissionid))
...
x_pred <- filter(simulate_dat$x_pred, admissionid == pt_admissionid)$time
...
saveRDS(eta_sim, paste0(dir, "/etasim",pt_admissionid, ".rds"))
...
df_pred <- df_pred %>%
  ...
  mutate(value = ifelse((var %in% 6:8 | var == 10) & rn %% 4 != 1,
                        NA,
                        value)) %>%
  ungroup() %>%
  select(-rn, -mean, -sd)
...
saveRDS(df_pred, file = paste0(dir, "/y_pred",
                               pt_admissionid, ".rds"))
print(paste0("trajectory saved for ", pt_admissionid))
return(pt_admissionid)
}

# load patients

patients <- select(amd_b, admissionid)
...
finished_patients <- foreach(k = 1:nrow(patients)) %dopar% simulate_trajectory(patients$admissionid[k])
