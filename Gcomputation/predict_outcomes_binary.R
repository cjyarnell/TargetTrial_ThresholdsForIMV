...
source("models/predict_functions.R")

dir = "mimic_simulatedTrajectories"
dirPred = "mimic_predictedOutcomes/"
...
M = 20
D_c <- 9
D_b <- 5 # don't include the observed events variables (dc, death, imv)
...
# load bart models
bart_mods <- list(
    M0 = readRDS("models/bart_binM0.rds"), # death28
    M1 = readRDS("models/bart_binM1_death.rds")) # imv28


# load baseline data for the patients
bart_X <- readRDS("data/bart_Xbin.rds") %>%
    select(-id,-partition,
           -dnr,-aml,
...
# rules 
rule_type <- list("SF", "SF","SF", "SFRR","SFRR","SFWoB", "SF","SF", "Delta","Delta","MultiOrgan_Or", "MultiOrgan_And", "UC")
rule_thresholds <- c(98, 88,  110, 25, 35, 98, 98, 98, 1, 0.5,1,1,1)
rule_durations <-  c(  1, 1,    1,  1,  1,  1,  2,  3, 1,   1,1,1,1)
...
temp_outcomes_calc <- function(k_stay_id, y_pred, iterstart, iterdiv){
     
# container for temporary outcomes for each trajectory
      temp_outcomes <- data.frame(stay_id = k_stay_id,
   ...
   #                  n_mark,
    ...
    #                        "pressor",
     #                       "neuro",
          ...
          if(i %% 10 == 0){print(paste("patient", k_stay_id, "| iter", i))}
...
sim_traj_rules_tsf_pred <- function(k, div){
        ...
        # k = 38 # for testing
    k_stay_id <- patients$stay_id[k]
    
    y_pred <- load_y_pred(k_stay_id)
        
    print(paste0("loaded trajectories for stay_id ", k_stay_id))
    
    iterstarts <- seq(from = 1, to = iter, by = iter/div)
    iterdiv <- iter/div
    
    temp_outcomes <- foreach(j = 1:div) %dopar% temp_outcomes_calc(k_stay_id, 
                                                                   y_pred, 
                                                                   iterstarts[j], 
                                                                   iterdiv)
    ...
    # load baseline and time series features 
        X_k <- filter(bart_X, stay_id == k_stay_id) %>%
            select(-stay_id,
               -death28,
               -imv28,
               -imv, -eventtime,
    ...
    print(paste("patient", k, ", stay_id =", k_stay_id, "complete"))
...
outcome_list <- foreach(k = start:(start+cycle-1)) %dopar% sim_traj_rules_tsf_pred(k,div)
